...
function erstellePersonenOverlay(titel, callback, ausgeschlossene = []) {
  ...
  <div style="display:flex; gap:20px; flex-wrap:wrap; justify-content:center; max-width:90%;">
    ...
    `<button class="spieler-btn" style="padding:25px 40px; font-size:2rem; background:#333; color:white; border:none; border-radius:30px; cursor:pointer; display:flex; align-items:center; justify-content:center; box-shadow: inset 0 -8px 12px -6px #0008, inset 0 8px 12px -6px #fff4;" onclick="${callback}('${s}')">${s}</button>`
    ).join("")}
  </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:15px;max-width:80%;">
      ${[4,5,6,7].map(n => 
        `<button onclick="felderBestaetigt('${person}', ${n})"
            style="padding:30px 30px; font-size:2.5rem; background:#ff4757; border:none; border-radius:15px; line-height:1.2; padding-top:23px; display:flex; align-items:center; justify-content:center; min-width:80px;">
            ${n}
         </button>`
      ).join("")}
    </div>
