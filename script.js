...
// Funktion für das Diagramm
function updateDiagramm() {
  const ctx = document.getElementById('trinkChart')?.getContext('2d');
  if (!ctx || spieler.length === 0) return;

  const labels = spieler;
  const schluecke = labels.map(name => trinkCounter[name]?.schluecke || 0);
  const exen = labels.map(name => (trinkCounter[name]?.exen || 0) * 10); // Exen als 10er-Balken

  if (trinkChart) trinkChart.destroy();

  trinkChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: labels,
      datasets: [
        {
          label: 'Schlücke',
          data: schluecke,
          backgroundColor: '#ffa502', // Orange
          borderRadius: 10,
        },
        {
          label: 'Exen (×10)',
          data: exen,
          backgroundColor: '#ff4757', // Rot
          borderRadius: 10,
        }
      ]
    },
...
let gesamtVisible = false; // Erscheint nach erstem Reset
...
// Beim Trinken → aktuell + gesamt addieren
function addToCounters(person, schluecke = 0, exen = 0) {
  trinkCounter[person].schluecke += schluecke;
...
// In resetBestaetigt() – Gesamt bleibt, Container zeigen
function resetBestaetigt() {
  // ... dein alter Code ...
  if (!gesamtVisible) {
...
// Zweites Diagramm updatet
function updateGesamtDiagramm() {
  const ctx = document.getElementById('gesamtChart')?.getContext('2d');
  if (!ctx || spieler.length === 0) return;

  const labels = spieler;
  const schluecke = labels.map(name => gesamtCounter[name]?.schluecke || 0);
  const exen = labels.map(name => (gesamtCounter[name]?.exen || 0) * 10);

  if (gesamtChart) gesamtChart.destroy();

  gesamtChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: labels,
      datasets: [
        { label: 'Schlücke', data: schluecke, backgroundColor: '#00d2d3', borderRadius: 10 },
        { label: 'Exen (×10)', data: exen, backgroundColor: '#ff4757', borderRadius: 10 }
      ]
    },
    options: {
      responsive: true,
...
// In updateDiagramm() am Ende hinzufügen
function updateDiagramm() {
  // ... dein alter Code ...
  if (gesamtVisible) updateGesamtDiagramm();
}
